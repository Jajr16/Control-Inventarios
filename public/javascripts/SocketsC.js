...
if (pathname == "/users/altasPro") {
    ...
    if (tok == "4dnM3k0nl9s" || tok == "4dnM3k0nl9z" || tok == "4dnM3k0nl9A" || tok == "FGJYGd42DSAFA" /*TEMPOTAL*/) {
        ...
        socket.emit("Consul_EqpExist");
        // Consulta de equipos
        socket.on('Desp_EquiposExist', async (data) => {
            console.log('Datos recibidos:', data.Num_Serie);

            document.querySelector("#DatosEquipos tbody").innerHTML += `
                <td id="Num_Serie">${data.Num_Serie}</td>
                <td id="Equipo">${data.Equipo}</td>
                <td id="Marca">${data.Marca}</td>
                <td id="Modelo">${data.Modelo}</td>
                <td id="Nom">${data.NombreEmp}</td>
                <td id="Ubi">${data.Ubi}</td>
                <td id="Eliminar" class="BotonER" onclick="Abrir1()">Sacar Equipos</td>
                </tr>
            `;
        });

        // Sacar equipo existente
        socket.on('RBEE', async () => {
            let EliminarEquiposExist = document.getElementsByClassName("BotonER");

            for (let i = 0; i < EliminarEquiposExist.length; i++) {
                EliminarEquiposExist[i].addEventListener("click", existRespEqp);
            }
            function existRespEqp(e) {
                var elementosTD = e.srcElement.parentElement.getElementsByTagName("td");

                for (let i = 0; i < elementosTD.length; i++) {
                    // obtenemos cada uno de los valores y los ponemos en la variable "valores"
                    valores0E = elementosTD[0].innerHTML;
                    valores1E = elementosTD[1].innerHTML;
                    valores2E = elementosTD[2].innerHTML;
                }

                document.querySelector("#TituloEliminar").innerHTML = `Se generará una responsiva del equipo "${valores1E}" de marca "${valores2E}"`;

                const formEquipoExistBaja = document.querySelector("#RespEquipos");
                formEquipoExistBaja.addEventListener("submit", EnviarResp);
                function EnviarResp(e) {
                    e.preventDefault();
                    if ($("#Num_Serie").val() != "" && $("#NombreEmp").val() != "") {
                        socket.emit('RespEquiposExist', { Num_Serie: valores0E, Emp: $("#NombreEmp").val() });

                        socket.once('Responsiva_Realizada', function (Respuesta) {
                            alert(Respuesta.mensaje);
                            location.reload();
                        });
                        socket.once('Fallo_ResponExist', function (Respuesta) {
                            alert(Respuesta.mensaje);
                            location.reload();
                        });
                    }
    ...
    if (tok == "4dnM3k0nl9s" || tok == "4dnM3k0nl9z" || tok == "4dnM3k0nl9A" || tok == "FGJYGd42DSAFA" /*TEMPOTAL*/) {
        ...
        socket.emit("Consul_MobExist");
        // Consulta de equipos
        socket.on('Desp_MobiliarioExist', async (data) => {
            console.log('Datos recibidos:', data.Num_Serie);

            document.querySelector("#DatosMobi tbody").innerHTML += `
                <td id="Num_Inventario">${data.Num_Inventario}</td>
                <td id="Descripcion">${data.Descripcion}</td>
                <td id="Nom">${data.NombreEmp}</td>
                <td id="Eliminar" class="BotonER" onclick="Abrir1()">Sacar Mobiliario</td>
                </tr>
            `;
        });

        // Sacar equipo existente
        socket.on('RBME', async () => {
            let EliminarEquiposExist = document.getElementsByClassName("BotonER");
            ...
            function existRespEqp(e) {
                ...
                document.querySelector("#TituloEliminar").innerHTML = `Se generará una responsiva del mobiliario "${valores1E}"`;

                const formMobiliarioExistBaja = document.querySelector("#RespMobiliario");
                formMobiliarioExistBaja.addEventListener("submit", EnviarResp);
                function EnviarResp(e) {
                    e.preventDefault();
                    if ($("#Num_Inventario").val() != "" && $("#NombreEmp").val() != "") {
                        socket.emit('RespEquiposExist', { Num_Inventario: valores0E, Emp: $("#NombreEmp").val() });
...
} else if (pathname == "/users/ResSacEqp") {
    if (tok == "4dnM3k0nl9s" || tok == "4dnM3k0nl9z" || tok == "4dnM3k0nl9A" || tok == "FGJYGd42DSAFA" /*TEMPOTAL*/) {

        //Desplegar responsivas existentes
        socket.emit("Consul_RegProSac");
        // Consulta de productos
        socket.on('Desp_RespEqp', async (data) => {
            console.log('Datos recibidos:', data.Num_Serie);

            document.querySelector("#DatosEqpSac tbody").innerHTML += `
                <tr>
                    <td>${data.Num_Serie}</td>
                    <td>${data.NombreEmp}</td>
                </tr>
        ...
        // Crear pdf de responsivas de equipos
        function RespEquipoPDF() {
        }
...
} else if (pathname == "/users/ResSacMob") {
    if (tok == "4dnM3k0nl9s" || tok == "4dnM3k0nl9z" || tok == "4dnM3k0nl9A" || tok == "FGJYGd42DSAFA" /*TEMPOTAL*/) {

        //Desplegar responsivas existentes
        socket.emit("Consul_RegMobSac");
        // Consulta de productos
        socket.on('Desp_RespMob', async (data) => {
            console.log('Datos recibidos:', data.Num_Inventario);

            document.querySelector("#DatosMobSac tbody").innerHTML += `
                <tr>
        ...
        // Crear pdf de responsivas de mobiliario
        function RespMobiliarioPDF() {
        }
