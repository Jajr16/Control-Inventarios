...
io.on('connection', (socket) => {
    ...
    socket.on('Cambios_Prod', async (data, dataOld) => {
        ...
        //Se agrega productos a la BD
        db.query('update almacen set Cod_Barras = ?, Categoria = ?, Articulo = ?, Marca = ?, Descripcion = ?, Unidad = ?, Existencia = ? where Cod_Barras = ?', [data.CodBarras, data.Cate, data.Producto, data.Marca, data.Descripcion, data.Unidad, data.Existencia, dataOld.CBO], function (err2, result) {
            if (err2) console.log("Error de inserción de productos: ", err2);
            if (result) {
                db.query('update Facturas_Almacen set Num_Fact = ?, Ffact = ?, Cod_Barras = ?, FIngreso = ?, Proveedor = ? where Num_Fact = ?', [data.NumFactura, data.FechaFac, data.CodBarras, data.FecAct, data.Proveedor, dataOld.NFO], function (err1, result) {//Insertar factura
                    if (err1) console.log("Error en inserción de facturas: ", err1);
                    if (result) {
                        console.log("Resultado de inserción de productos: ", result);
