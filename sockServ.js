            ...
            if (result.length > 0) {//Si sí hizo una búsqueda
                ...
                db.query('insert into equipo values(null,?,?,?,?,(select Num_emp from empleado where Nom = ?),?)', [data.Num_S, data.Equipo, data.MarcaE, data.ModelE, data.NomEn, data.UbiE], async function (err, result) {
                    ...
                    if (result) {
                        ...
                        db.query('SELECT * FROM Equipo LEFT JOIN PCs ON Equipo.Num_Serie = PCs.Num_Serie LEFT JOIN Monitor ON Equipo.Num_Serie = Monitor.Num_Serie LEFT JOIN Mouse ON Equipo.Num_Serie = Mouse.Num_Serie LEFT JOIN Teclado ON Equipo.Num_Serie = Teclado.Num_Serie LEFT JOIN Accesorio ON Equipo.Num_Serie = Accesorio.Num_Serie WHERE Num_emp = ? and Equipo.Num_Serie = ?;', [num_emp, data.Num_S], function (err, res) {
                            if (err) console.log("Error de inserción de productos: ", err);
                            if (res) {
        ...
        db.query('select*from mobiliario', function (err, result) {
            ...
            if (result.length > 0) { //Si sí hizo una búsqueda
                db.query('delete from mobiliario where Descripcion = ?', data, function (err, result) {
                    ...
                    console.log(data);
                    if (result.affectedRows > 0) {
                        console.log("Resultado de eliminacion de mobiliario: ", result);
